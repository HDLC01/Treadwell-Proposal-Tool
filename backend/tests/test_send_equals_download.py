...
every view, through whatever code was deployed that day. Now:
  ...
  * one server function, `_render_documents`, renders a saved payload for the Files page's
    ...
    memoised under the payload's hash so a Download and a Send hand out the SAME bytes;
...
def test_a_download_after_the_send_is_the_file_the_customer_has(world):
    """The other order: send first (the send renders and freezes the PDF), then download. The
    estimator's copy is the customer's copy."""
...
def test_an_unreadable_documents_table_falls_back_to_the_render(world, monkeypatch):
    """A database the DDL has not reached yet must not take every customer's PDF down: the read
    fails soft to the re-render every revision used before. (Writes are the strict half.)"""
    world.store.setdefault("draft_revisions", []).append(
        {"project_id": "d1", "revision_no": 1, "created_by": EST,
         "data": {"project_name": "Old", "proposal_payload": copy.deepcopy(PAYLOAD)}})
