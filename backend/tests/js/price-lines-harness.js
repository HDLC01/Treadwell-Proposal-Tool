...
function build(st, opts) {
  ...
  const body = [
    ...
    "         comboLinesForPayload, rebuildPricing, effectiveWorkType, renderProposalExtras, baseDescLabel };",
  ].join("\n");
...
// 15. TAX WORDING OF HIS OWN IS HIS (review, 2026-09-26). Only TODAY's wording on a line with a place
...
// 16. A HAND-TYPED PRICE THAT QUOTES TODAY'S FIGURE (review, 2026-09-26): "$5,800 – … (discounted
...
//    quote. It used to become the live amount: no mark, no Send prompt, and the quote moved.
{
  ...
  out.quoted = { stored: st.price_overrides.lines2.base, cls: base.className, title: base.title,
                 ...
                 ask: TWPrice.ownFigureQuestion(warn, "send") };
  st.proposal_lump_sum = 9100;
  api.refreshPriceDisplay();
...
// 17. A COMBO "Option N" LINE RE-WORDED BEFORE THE LIVE SHAPE (review, 2026-09-26). Under
