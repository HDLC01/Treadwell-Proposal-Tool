...
over rows shaped like the ones production serves, on all three tabs in both views — with ONLY the
names portal.js actually binds in scope. Any identifier the page uses without importing is an
immediate ReferenceError, which is the entire point. Deliberately not jsdom: a stubbed global would
let exactly this bug hide again.
...
COMBOS = [(t, v) for t in ("active", "test", "lost") for v in ("board", "table")]
...
def test_the_live_tabs_draw_the_pipeline_columns(rendered):
    for tab in ("active", "test"):
        ...
        assert r["columns"] == 8, (
            "the %s board drew %s columns; the pipeline has 8" % (tab, r["columns"]))
...
@needs_node
def test_the_new_button_is_on_the_live_tabs_and_not_on_lost(rendered):
    assert rendered["results"]["active/board"]["newButton"]
    assert rendered["results"]["test/board"]["newButton"]
    assert not rendered["results"]["lost/board"]["newButton"], (
        "+ New is on the Lost board, where it would file a new bid as closed lost")
