...
def test_a_layout_saved_before_content_versions_survives_a_deploy(ran):
    """A pre-hash stamp is the file's mtime, which every deploy moved without changing a byte, so
    the old strict compare threw every saved layout away after each deploy. It is honoured when
    taken at or after the second this content landed; before that, or with no floor (the content
    changed since), it is still refused."""
    assert ran["restoreLegacyAfterFloor"] == [[3, {"h_pt": 300}]]
    assert ran["restoreLegacyBeforeFloor"] == []
    assert ran["restoreLegacyNoFloor"] == []
