...
_PREQUAL_SYSTEM_PROMPT = (
    ...
    "- recommendation answers ONE question: is there a scope Treadwell "
    "self-performs? \"pass\" when there is none (roofing, drywall-only, "
    "sitework, carpet/VCT-only, waterproofing-only, etc.). \"review\" when the "
    "scope is plausible but unclear — a blanket all-trades invite, or an email "
    "too thin to tell. \"pursue\" when a concrete epoxy / polish / gypsum scope "
    "is actually named.\n"
    "- Do NOT let distance, deadline or job size move the recommendation. They "
    "belong in fit_score, which is the whole picture: a named flooring scope an "
    "hour away scores high, the same scope four hours away scores low, and both "
    "are still \"pursue\" because the work is ours either way. Keeping the two "
    "separate is what lets a human filter on one and a threshold on the other.\n"
