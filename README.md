...
Set in `/opt/treadwell/.env` (prod) / `.env.staging` (staging). **Never
committed.** A few non-secret values (DB path, Claude config dir, output
folder) are set directly in the compose files instead.

| Var | Required | Purpose |
|---|---|---|
| `SUPABASE_URL` | Yes | Cloud Supabase project URL — Google auth + JWT verification (JWKS). |
| `SUPABASE_SERVICE_ROLE_KEY` | Yes | Server-side key for data access + auth-admin ops. Secret; never sent to the browser. |
| `SUPABASE_ANON_KEY` | Yes | Publishable key handed to the frontend for sign-in. |
| `AUTH_ALLOWED_DOMAIN` | No (default `wetreadwell.com`) | Email domain allowed to sign in. |
| `CLAUDE_CODE_OAUTH_TOKEN` | Yes (for AI Autofill) | Long-lived OAuth token (`claude setup-token`, ~1 yr) that authenticates the in-container `claude -p` CLI. |
| `SUPABASE_JWT_SECRET` | Only for HS256 tokens | Legacy shared secret if the project signs JWTs HS256. |
| `SUPABASE_DATA_URL` / `SUPABASE_DATA_KEY` | Staging only | Point the DATA store at the VPS PostgREST instead of cloud Supabase. |
| `DRAFTS_DB_PATH` | Set in compose | Prod SQLite drafts DB path (on a persistent volume). |
| `DROPBOX_*`, `ANTHROPIC_API_KEY` | Inert / legacy | The Dropbox upload path was retired (2026-06-09) and `claude -p` replaced the API-key autofill; left in place but unused on the live path. |
